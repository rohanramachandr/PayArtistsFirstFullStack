...
// import {theme as ThemeContext} from "../theme";
...
const ResponsivePlayingBar = ({music}) => {
    const [bannerOpen, setBannerOpen] = useState(false);
    const [currentlyPlaying, setCurrentPlaying] = useState();
    // const [{id, name, author_name,}, setCurrTrack] = useState(music);
    const id = 1;
    const name = "rohan";
    const author_name = "Rohan trio";
    ...
    return (
        <div style={useStyle.component} className={"footer-player"}>
            ...
            <Button
                    startIcon={<Avatar variant="square" src={loadingArtwork} alt={name}/>}
